...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana muestra por debajo del limite superior esperado. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
Al analizar la circulación viral entre las semanas epidemiológicas 21 a 23, comparando los periodos 21–20 versus 22–23, se observan variaciones importantes en la actividad de los principales virus respiratorios. Rhinovirus, aunque mostró una leve disminución (28.9% a 28.0%), se mantuvo como el virus con mayor proporción en circulación, mientras que Enterovirus aumentó su frecuencia de 13.3% a 24.0%, lo cual sugiere un incremento reciente en su actividad.

\newpage

Por el contrario, Parainfluenza virus (1, 2 y 3) no presentó actividad en el periodo más reciente (sem. 22–23), tras haber circulado previamente (6.7%), lo que podría indicar un cese temporal en su transmisión. En el caso de Metaneumovirus, la proporción descendió ligeramente (de 15.6% a 12.0%), manteniendo una circulación activa. Influenza A mostró una reducción de 8.9% a 4.0%, mientras que VSR se mantuvo estable (11.1% a 12.0%). Adenovirus evidenció un aumento leve (6.7% a 8.0%).

Llama la atención la ausencia de circulación de Coronavirus y SARS-CoV-2, con 0.0% en ambos periodos recientes. Influenza B mostró un leve repunte, pasando de 0.0% a 4.0%, al igual que Influenza AH1N1, que se mantuvo en 4.4%.

Estos resultados reflejan una dinámica viral variable, con descenso en la actividad de algunos virus respiratorios como Parainfluenza e Influenza A, y un aumento o persistencia en otros como Enterovirus, Rhinovirus y Metaneumovirus. Esta información es fundamental para orientar la vigilancia virológica y fortalecer las medidas de prevención, particularmente en poblaciones pediátricas y grupos vulnerables.

Las estimaciones presentadas corresponden a muestras recolectadas en pacientes que consultaron a servicios de urgencias o se encontraban hospitalizados. Por tanto, los resultados reflejan la circulación viral en una población con mayor compromiso clínico y cuyo panel diagnóstico está orientado al soporte en la toma de decisiones terapéuticas (por ejemplo, uso racional de antibióticos), lo cual puede no representar el comportamiento general en la comunidad.
...
valor <- predict.glm(model, newdata = data.frame(t= c(25)),type = "response") #cambiar a cada semana
