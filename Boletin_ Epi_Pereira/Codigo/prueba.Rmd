...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 42 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=42) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
#   summarise(sum(tot_irauci)) %>%
#   ungroup() %>%
#   pivot_wider(names_from = semana, values_from = `sum(tot_irauci)`)
```
...
varicela_ajus <- expand.grid(semana = 1:42, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
