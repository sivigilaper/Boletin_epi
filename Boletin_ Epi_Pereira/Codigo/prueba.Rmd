...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 26 - 2023} 
...
infe<- evento_995 %>% 
 ...
 ######### QUITAR PARA LA PROXIMA SEMANA  ######### QUITAR PARA LA PROXIMA SEMANA 
#infe[25,"c_hosp"] <- 52       
infe[24,"c_hosp"] <- 60
 ######### QUITAR PARA LA PROXIMA SEMANA  ######### QUITAR PARA LA PROXIMA SEMANA 
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana presenta comportamiento dentro de lo esperado, en la semana 25 se presenta un comportamiento inusual verificado, el cual corresponde a dato incorrecto y este sera ajustado en proxima semana epidemiologica (Figura 2). La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), muestra un comportamiento de disminución. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
filmarray2 %>% 
  ...
  labs(x = "Semana epidemiologica",
       ...
       fill = "Resultado")+
  theme_classic()+
  theme(legend.position = "top", axis.text.x = element_text(size = 6), legend.text = element_text(size = 7), legend.title = element_text(size = 8))+
  scale_fill_manual(values = c("#5189C1","#95CFA5"))
```
\newpage
A periodo epidemiologico 5 de 2023 `r nrow(filmarray2) ` muestras en el municipio, con aislameintos tanto virales como bacterianos con una positividad acumulada de `r positivo[2,2] `  y una  positividad semanal como lo muestra la **figura 5**. La incidencia de microorganismos en aislamientos virales procesado por FilmArray para lo corrido del año son:
...
aislamiento %>% 
  ...
  theme_classic()+
    theme(legend.title = element_blank(), legend.position = "bottom", axis.text.x = element_text(size=6), legend.text = element_text(size = 6))+
  scale_fill_manual(values = c("#A6C7F0","#95CFA5","#4BA52F","#E43149","#A600F0","#5FA1D4","#9767BB","#EC702D","#4174AD","#6DBFBE","#ED70C3"))
```
...
varicela_ajus <- expand.grid(semana = 1:26, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
