...
subtitle: "Alertas Tempranas Semana Epidemiologica 3 de 2022"
header-includes:
...
epi3 <- epi3 %>% 
  ...
  mutate(cod_dpto_r= as.numeric(cod_dpto_r)) %>% 
  mutate(fecha_nto_= as.numeric(fecha_nto_)) %>% 
  mutate(tel_dil_f_= as.numeric(tel_dil_f_))
...
epi3_1 <- epi3 %>% 
  ...
  filter(as.numeric(semana)<=3)      # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
...
En relación a la notificación segun servicio y comparando la semana notificada del actual año con la misma semana del año anterior, se identifica una disminución en la hospitalización IRA en UCI, pero con aumentos relativos de $8.3 \%$  y $70\%$ en sala general y urgencias, respetivamente.
