...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 35 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=35) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
#   summarise(sum(tot_iraext)) %>% 
#   ungroup() %>% 
#   pivot_wider(names_from = semana, values_from = `sum(tot_iraext)`)
...
varicela_ajus <- expand.grid(semana = 1:35, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
