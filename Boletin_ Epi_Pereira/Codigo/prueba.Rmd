...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 8 - 2023} 
...
epi3_1 <- epi3 %>% 
  ...
  filter(as.numeric(semana)<=8)      # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
...
De esta manera se estima que la incidencia acumulada para microorganismos asialdos a traves de panel molecular viral rapido en orden de aparición son  el Rinovirus, enterovirus. Como causa comun de bronquiloitis en menores de 2 años el Virus Sincitial Respiratorio (VSR).
...
```{r, echo=FALSE, warning=FALSE, fig.height=3.2 , fig.width=7, fig.cap="Circulación virus respiratorios, Pereira 2022"}
aislamiento %>% 
  mutate(viral = case_when(value == "Enterovirus" ~ "Enterovirus",
                                         value == "Rhinovirus"~ "Rhinovirus",
                                         value == "Rinovirus"~ "Rhinovirus",
...
plot(incidence(as.Date(viruela_curva$fis), groups = viruela_curva$clasificacion_final_del_caso, first_date = "2022-07-10", last_date = "2023-02-28", interval = 7), border = "black")+ theme_classic()+
  coord_cartesian(ylim = c(0,4))+ labs(title = "Curva epidemica casos viruela simica", x = "Fecha inicio de sintomas", y = "Casos")+ scale_x_date(date_breaks = "7 day", date_labels = "%d %b %y")+
