...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 18 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=18) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general muestra un incremento por encima del limite superior (Figura 2), se encuentra al momento en verificacion del incremento inusual detectado. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), muestra un comportamiento de incremento hacia umbral estacional. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
Por grupo de edad en la hospitalizaciones por IRA, la mayor concentración de hospitalizaciones a causa de infección respiratoria se da en el grupo de edad menores a un año y el grupo de 1 año.
...
varicela_ajus <- expand.grid(semana = 1:18, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
