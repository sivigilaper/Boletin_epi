...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana disminuye el registro de casos pero se encuentra por debajo umbral estacional. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
legend(30,20, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2024"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.5)
```
...
ajuste <- casos_sem2['210',(length(casos_sem2['210',])-5):(length(casos_sem2['210',])-1)]
p <- data.frame(t = as.numeric(names(ajuste)),ajuste)
model <- glm(ajuste~t, family = "poisson", data = p)
valor <- predict.glm(model, newdata = data.frame(t= c(12)),type = "response")
plot(canalend$semana, canalend$exito, type='l', ylim=c(0,100), xlim=c(1,52), col="green",lwd=2, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
legend(30,90, c("IC inf 95%","Media", "IC sup 95%", "2024","Ajuste por rezago"), col=c("green","yellow","red","black","blue"), lty = c(1,1,1,1,1), lwd = c(1,1), bty="n", cex=0.6)
```
...
Durante este año 2024, se han notificado a semana epidemiologica `r ncol(casos_sem)` al sistema de vigilancia SIVIGILA un total de `r covid[which(covid$tip_cas_=="3"),"n"]` casos leves o sin necesidad de hospitalización, para esta ultima semana epidemiologica no hya reporte de casos para COVID leve.
