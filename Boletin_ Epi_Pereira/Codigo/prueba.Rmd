...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 6 - 2023} 
...
evento_995<- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 995 SEM 06.xlsx")
evento_998 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 998 SEM 06.xlsx")
...
varicela <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 831 datos basicos SEM 06.xlsx")
filmarray <- read_sheet("https://docs.google.com/spreadsheets/d/1W9bwlXzatRVZKkwsaVo2i91fs_RcpSAQ77Ks99ykyRc/edit#gid=1910681332", sheet = "Muestras2022")
...
epi3_1 <- epi3 %>% 
  ...
  filter(as.numeric(semana)<=6)      # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
...
epi2<-epi2 %>% 
  filter(as.numeric(semana)<=6)   # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
epi1 <- epi1 %>% mutate(semana = as.numeric(semana))
epi2 <- epi2 %>% mutate(semana = as.numeric(semana))
...
```{r, echo=FALSE, fig.cap="Canal endémico IRA , Pereira 2023", fig.height=3, fig.width=7, fig.pos='h'}
plot(ira$semana, ira$icinftotal, type='l', ylim=c(1,4200), xlim=c(1,52), col="green",lwd=3, xlab="Semana epidemiológica", ylab="Casos", cex=0.6)
...
legend(25,3200, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "casos 2023"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general muestra un comportamiento de ascenso (Figura 2), pero en una posicion bajo el umbral estacional. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.

```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en sala general , Pereira 2023", fig.height=3, fig.width=7}
...
legend(25,2750, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "casos 2023"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), muestra un comportamiento estable en UCI por IRA. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en UCI , Pereira 2023", fig.height=3, fig.width=7}
...
legend(8,25, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2023"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.5)
```

Por grupo de edad en la hospitalizaciones por IRA, la mayor concentración de hospitalizaciones a causa de infección respiratoria se da en el grupo de edad menores a un año.
...
```{r, echo=FALSE, fig.cap="Canal endémico consulta externa y urgencias por IRA, Pereira 2023", fig.height=3, fig.width=7}
...
En cuanto a la edad, en la consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se observa una concentración del mas de 10% en los de un 1 año y menos, con una mayor concentración de la consulta por IRA entre los menores a 1 año. El canal endemico general para IRA por debajo de  un umbral estacional
...
```{r, echo=FALSE, warning=FALSE, fig.height=2.5 , fig.width=7, fig.cap="Positividad muestras tracto respiratorio, Pereira 2022"}
filmarray %>% 
  group_by(semana) %>% 
...
como cierre para el año 2022 bajo la implementacion de vigilancia por laboratorio de instituciones habilitadas y con tecnologia de panel viral molecular rapido se recolectaron durante el año `r nrow(filmarray) ` muestras en el municipio, con aislameintos tanto virales como bacterianos con una positividad acumulada de `r positivo[2,2] `  y una  positividad semanal como lo muestra la **figura 5**.

El consolidado anual las prevalencias de aislamientos virales para el año 2022 son las siguientes:
...
aislamiento %>% 
  ...
  group_by(viral) %>% 
  summarise(n = n(),
            prevalencia = round(n/633*100,1),
            IC95inf = round(binom::binom.agresti.coull(n,633)[,5]*100,2),
            sup = round(binom::binom.agresti.coull(n,633)[,6]*100,2)) %>% 
  arrange(desc(n)) %>%
  filter(n>=8) %>% 
  flextable::flextable() %>% 
  flextable::autofit() %>% 
...
De esta manera se estima que la prevalencia en microorganismos asialdos a tarves de panel molecular viral en orden de aparción son  el Rhinovirus, enterovirus y causa comun de bronquiloitis en menores de 2 años el Virus Sincitial Respiratorio (VSR).

Cabe resaltar que las estimaciones presentadas corresponden a muestras recolectadas en pacientes consultantes a servicios de urgencias y hospitalizados, lo cual puede ser un reflejo de una circulación viral entre pacientes que requieren hospitalización y que dicho panel es realizado con el fin de guiar el manejo de terapia antibiotica y pueden no reflejar la circualción de microorganismos en pobalcion general no consultante.

Sin embargo permite para la vigilancia de virus respiratorios establecer la existencia de microorganismos dentro de nuestra población causantes de pico estacionarios de infección respiratoria aguda como los producidos en meses de junio-julio y y octubre-noviembre de cada año.
...
En la semana epidemiológica `r ncol(casos_sem)` de 2023 se notificaron al Sivigila `r casos_sem["300",ncol(casos_sem)]` casos de agresiones por animales potencialmente transmisores de rabia , con un acumulado hasta el momento de `r sum(casos_sem["300",1:ncol(casos_sem)])` casos.
...
Entre los tipos de cáncer objeto de vigilancia en salud pública se encuentra cáncer de mama y cuello uterino. De este evento esta semana se notificaron 0 casos, con un acumulado hasta el momento de 0 casos . En Cáncer en menor de 18 años no se han reportado a la fecha  casos.

```{r, echo=FALSE}
# `r casos_sem["155", ncol(casos_sem)]` casos (figura 1)
# `r sum(casos_sem["155",1:ncol(casos_sem)])`
...
varicela_ajus <- expand.grid(semana = 1:5, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
...
plot(incidence(as.Date(viruela_curva$fis), groups = viruela_curva$clasificacion_final_del_caso, first_date = "2022-07-10", last_date = "2023-01-31"), border = "black")+ theme_classic()+
  coord_cartesian(ylim = c(0,4))+ labs(title = "Curva epidemica casos viruela simica", x = "Fecha inicio de sintomas", y = "Casos")+ scale_x_date(date_breaks = "10 day", date_labels = "%d %b %y")+
