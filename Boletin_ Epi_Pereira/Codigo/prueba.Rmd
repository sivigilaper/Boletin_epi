...
evento_995 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/2026_995.xlsx")
evento_998 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/2026_998.xlsx")
...
varicela <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/2026_831_Reporte.xlsx")
...
2
filmarray2 <- add_case(filmarray,c_viral) # consolidado total de filmarray PEREIRA
...
diarreas <- evento_998 %>%
  group_by(semana) %>%
  summarise(casos = sum(cas_conc, na.rm = TRUE))
...
saveRDS(casos2022,
        "/cloud/project/Boletin_ Epi_Pereira/Datos/casos2022.rds")
...
A semana epidemiológica `r ncol(casos_sem)` de 2026 se notificaron un total entre consultas externa/urgencias hospitalizaciones en sala y hospitalizaciones en UCI de `r format(consultas,scientific = F)` de las cuales el `r round(infecciones/consultas*100, digits = 2)`% (n= `r format(infecciones, scientific = F)`) corresponden a IRA. El canal endémico muestra el evento para la semana `r ncol(casos_sem)` bajo el umbral estacional.

```{r, echo=FALSE, fig.cap= paste("Canal endémico IRA , Pereira 2026","semana epidemiologica",epiweek), fig.height=3.3, fig.width=7, fig.pos='h',}
...
legend(25,4000, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "Casos 2026"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

En relación a la notificación segun servicio y comparando la semana notificada del actual año con la misma semana del año anterior, se identifica un aumento en el servicio de hospitalización UCI.

**Tabla 1. Comportamiento de la Infección Respiratoria Aguda segun tipo de servicio, semana epidemiológica `r ncol(casos_sem)`**

| **Servicio**               |                 **a Sem `r ncol(casos_sem)`/2025**                  |                 **a Sem `r ncol(casos_sem)`/2026**                 |                                                                      **Variacion (%)**                                                                      |
|-----------------|:---------------:|:---------------:|:-------------------:|
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general se reporta por encima del  umbral estacional, en una zona de brote. Datos que seran verificados para la proxima semana epidemiologica. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.

```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en sala general , Pereira 2026", fig.height=3.3, fig.width=7}
plot(ira$semana, ira$icinfhosp, type='l', ylim=c(1,250), xlim=c(1,52), col="green",lwd=3, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
legend(5,200, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "Casos 2026"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), se encuentra sobre el umbral estacional esperado. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en UCI , Pereira 2026", fig.height=3.3, fig.width=7}
...
legend(30,20, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2026"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.5)
```

Por grupo de edad en la hospitalizaciones por IRA, la mayor concentración de hospitalizaciones a causa de infección respiratoria esta en el grupo menor a 5 años.
...
En consulta externa y urgencias se han reportado de manera acumulada `r format(sum(evento_995$tot_exturg), scientific = F)` de consultas, donde las consultas por infección respiratoria aguda ocupan el `r round(sum(evento_995$tot_iraext)/sum(evento_995$tot_exturg)*100,digits=3)`%. 

```{r, echo=FALSE, fig.cap="Canal endémico consulta externa y urgencias por IRA, Pereira 2026", fig.height=3.3, fig.width=7}
...
legend(25,3800, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2026"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

En cuanto a la edad, en las consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se muestra en la tabla. El canal endemico general para IRA esta por del umbral estacional en zona de brote.
...
aislamiento <- aislamientos %>% 
  ...
  add_case(aislamiento2) %>%              # une con base de paneles idime
  filter(fec_muestra >= "2026-01-04") %>%  # inicio año epidemiologico 2026
  mutate(semana = epiweek(fec_muestra)) %>%  
...
write_csv(aislamiento, "/cloud/project/Boletin_ Epi_Pereira/Datos/aislamientos_2026.csv")
  

positivo <- filmarray2 %>% 
  filter(fec_muestra >= "2026-01-04") %>%    # filtro de año de muestra
  group_by(resultado) %>% 
  summarise(n=n()) %>% 
...
df <- tibble(x = 50, y = 1, m_ips = list(m_ips))
```


```{r, echo=FALSE, warning=FALSE, fig.height=2.5 , fig.width=7, fig.cap="Positividad muestras tracto respiratorio, Pereira 2026"}
filmarray2 %>% 
  filter(fec_muestra >= "2026-01-04") %>% # filtra solo para año epidemiologico
  group_by(semana) %>% 
...
Durante el periodo analizado (semanas 6–5a y 7–8a), Rhinovirus se consolidó como el agente respiratorio predominante, con 52 aislamientos y una proporción de incidencia de 38,0% (IC95%: 30,26–46,31). Entre las dos semanas evaluadas se evidenció un incremento importante, pasando de 18,8% en la semana 6–5a a 39,1% en la semana 7–8a, lo que confirma una transmisión activa y en ascenso en la población muestreada.

En segundo lugar se ubicó Enterovirus, con 47 aislamientos y una proporción de incidencia de 34,3% (IC95%: 26,88–42,59). Este agente mostró un comportamiento similar al de Rhinovirus, con un aumento marcado de 18,8% a 39,1% entre ambos periodos semanales, lo que sugiere una co-circulación intensa y un incremento reciente de su participación relativa dentro de las muestras positivas.
...
En contraste, Influenza A presentó 15 aislamientos, con una proporción de incidencia de 10,9% (IC95%: 6,75–17,28), mostrando un leve descenso de 12,5% a 8,7% entre semanas. Por su parte, Adenovirus (8,8%; IC95%: 5,08–14,69) mantuvo una participación intermedia, con ligera variación ascendente (12,5% a 13,0%). El Virus Parainfluenza 1–3 (6,6%; IC95%: 3,49–12,01) mostró incremento desde ausencia de casos (0,0%) en la primera semana hasta 13,0% en la segunda, indicando reaparición reciente en el periodo evaluado.

\newpage

El Virus Sincitial Respiratorio (VSR) registró 9 aislamientos, con una proporción de incidencia de 6,6% (IC95%: 3,49–12,01), sin detección en ambas semanas evaluadas (0,0% en semana 6–5a y 0,0% en semana 7–8a), lo que sugiere ausencia de circulación activa en este corte temporal específico.

Otros virus como Coronavirus (5,8%; IC95%: 2,99–11,10) mostraron incremento de 6,2% a 13,0%, mientras que Influenza AH1N1 (5,8%; IC95%: 2,99–11,10) descendió de 12,5% a 0,0%. Finalmente, SARS-CoV-2 y Metaneumovirus presentaron proporciones bajas (0,7% cada uno), sin detección en las semanas analizadas.

En conjunto, el patrón observado refleja una circulación predominantemente sostenida y en ascenso de virus respiratorios no influenza, con claro liderazgo de Rhinovirus y Enterovirus, ambos con incrementos importantes en la segunda semana evaluada. Influenza A mantiene una circulación moderada sin tendencia ascendente marcada, mientras que VSR no evidencia actividad en el periodo analizado.

Este comportamiento sugiere un escenario de alta contribución de virus respiratorios distintos a influenza en la morbilidad respiratoria actual.

```{r, echo=FALSE, warning=FALSE, fig.height=2.5 , fig.width=7, fig.cap="Circulación virus respiratorios, Pereira 2026"}
aislamiento %>% 
  mutate(viral = case_when(value == "Enterovirus" ~ "Enterovirus",
                           value == "enterovirus" ~ "Enterovirus",
                           str_detect(value,pattern = regex("enterovirus humano", ignore_case = TRUE)) == TRUE ~ "Enterovirus",
...
```{r, echo=FALSE, fig.height=3, fig.width=7, fig.cap="Canal endémico Dengue, Pereira 2026"}
...
valor <- predict.glm(model, newdata = data.frame(t= c(12)),type = "response") #cambiar a cada semana
plot(canalend$semana, canalend$p25, type='l', ylim=c(0,52), xlim=c(1,52), col="green",lwd=2, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
lines(x = 12, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
...
En la semana epidemiológica `r ncol(casos_sem)` de 2026 se notificaron `r casos_sem["560",ncol(casos_sem)]` muertes perinatales; `r sum(casos_sem["560",1:ncol(casos_sem)])` casos acumulados hasta el momento.
...
Para esta semana se notificaron al Sivigila `r casos_sem["750", ncol(casos_sem)]` casos de sífilis gestacional, con un acumulado al momento de `r sum(casos_sem["750", 1:ncol(casos_sem)]) ` y casos de sífilis congénita para esta semana no se han notificado hasta el momento.
...
Durante este año 2026, se han notificado a semana epidemiologica `r ncol(casos_sem)` al sistema de vigilancia SIVIGILA un total de `r covid[which(covid$tip_cas_=="3"),"n"]` casos leves o sin necesidad de hospitalización, para esta ultima semana epidemiologica no hay reporte de casos para COVID leve.

```{r, echo=FALSE, warning=FALSE, fig.height=2 , fig.width=5.5, fig.cap="Notificacion semanal COVID-19, Pereira 2026"}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height= 4, fig.width= 4,fig.cap="Comportamiento de varicela para Hombres por grupos de edad, Pereira 2026"}
####################################################################################################
...
```{r, echo=FALSE, warning=FALSE, message=FALSE,fig.height= 4, fig.width= 4 ,fig.cap="Comportamiento de varicela para Mujeres por grupos de edad, Pereira 2026"}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Comportamiento inusuales en los eventos de interes en salud pública en el municipio de Pereira, Pereira 2026"}
...
poisson %>% 
  flextable() %>% 
  flextable::bg(j = 5, i = ~ `valor p`< 0.05, part = "body", bg = "red") %>% 
...
***Jorge Mario Estrada-Alvarez PhD. MSc. Epi***

Coordiandor Eje Estrategico Vigilancia en Salud Pública

***Maryluz Hincapie-Acuña MSc. Epi***

Baecteriologa Epidemiologa
