...
evento_995<-evento_995 %>%
  ...
  mutate(tot_ira= tot_irag+tot_irauci+tot_iraext)  %>% filter(semana<=52) # QUITAR SEMANA 
...
En cuanto a la edad, en las consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se observa una concentración del mas de 10% en los de un año. El canal endemico general para IRA se encuentra en zona limitrofe con el limite superior esperado.
...
El microorganismo con mayor incidencia hasta la fecha ha sido el Rhinovirus, con una incidencia acumulada del 18,4% (IC95%: 15,5 - 21,8), seguido por el Enterovirus con 16,5% y el Virus Sincitial Respiratorio (VSR) con 13,1%. Se observan variaciones en la circulación de algunos virus entre las semanas epidemiológicas 45-46 y 47-48: mientras que el Rhinovirus no ha mostrado cambios significativos, el Enterovirus dismunyo su circulación. Por su parte, otros virus como el VSR no ha mostrado cambios, influenza A y B, parainfluenza muestran cambios al aumento en su probable circulacion, lo que correlaciona al momento con un aumento en la consultas por IRA a servicios de salud evidenciado en los canales endemicos.
...
lines(x = 52, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
