...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 28 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=28) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana presenta comportamiento dentro de lo esperado, semanas anteriores se detecto comportamientos inusuales por dos semanas con ajuste de casos (Figura 2). La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
Por grupo de edad en la hospitalizaciones por IRA, la mayor concentración de hospitalizaciones a causa de infección respiratoria se da en el grupo de edad menores a un año y el grupo de 1 año y menores a 1 año de edad.
...
df <- tibble(x = 53, y = 1, m_ips = list(m_ips))
```
...
varicela_ajus <- expand.grid(semana = 1:28, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
