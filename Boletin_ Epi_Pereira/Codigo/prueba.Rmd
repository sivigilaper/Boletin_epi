...
#   summarise(sum(tot_ira, na.rm = TRUE)) %>%
#   ungroup() %>%
#   pivot_wider(names_from = semana, values_from = `sum(tot_ira, na.rm = TRUE)`)
```
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana incrementa el registro de casos superando el umbral estacional sobre el limite superior esperado. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
A periodo epidemiologico 4 de 2024 `r nrow(filmarray2 %>% filter(year(FECHA)==2024)) ` muestras en el municipio, con aislameintos tanto virales como bacterianos con una positividad acumulada de `r positivo[2,2] `  y una  positividad semanal como lo muestra la **figura 5**. La incidencia de microorganismos en aislamientos virales procesado por FilmArray para lo corrido del año son:
...
plot(canalend$semana, canalend$exito, type='l', ylim=c(0,200), xlim=c(1,52), col="green",lwd=2, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
lines(x = 21, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
