...
evento_995<-evento_995 %>%
  ...
  mutate(tot_ira= tot_irag+tot_irauci+tot_iraext)  %>% filter(semana<=45) # QUITAR SEMANA 
...
El microorganismo con mayor incidencia hasta la fecha ha sido el Rhinovirus, con una incidencia acumulada del 19.0% (IC 95%: 15.93% - 22.47%), seguido por el Enterovirus con 15.4% (IC 95%: 12.60% - 18.62%) y el Virus Sincitial Respiratorio (VSR) con 13.9% (IC 95%: 11.28% - 17.07%). Se observan variaciones en la circulación de algunos virus entre las semanas epidemiológicas 41-42 y 44-45:

Virus cuya circulación disminuyó:

Virus Sincitial Respiratorio (VSR) presenta reducción de 13.9% a 0%, Influenza AH1N1 con un descenso de 5.2% a 0%, Metaneumovirus con un descenso de 3.1% a 0%.

Virus cuya circulación aumentó:

Enterovirus incremento de 6.2% a 37.5%, Parainfluenza Virus aumento de 4.3% a 6.2%.
Adenovirus incremento de 3.1% a 6.2%.

Estos cambios sugieren un aumento considerable en la circulación de Enterovirus y Parainfluenza Virus, mientras que virus como el VSR, Influenza AH1N1 y Metaneumovirus han mostrado una disminución significativa en su incidencia reciente.
...
valor <- predict.glm(model, newdata = data.frame(t= c(45)),type = "response") #cambiar a cada semana
...
lines(x = 45, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
legend(1,240, c("IC inf 95%","Media", "IC sup 95%", "2024","Ajuste por rezago"), col=c("green","yellow","red","black","blue"), lty = c(1,1,1,1,1), lwd = c(1,1), bty="n", cex=0.6)
```
