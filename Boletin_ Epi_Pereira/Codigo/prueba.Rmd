...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 7 - 2023} 
...
evento_995<- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 995 SEM 07.xlsx")
evento_998 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 998 SEM 07.xlsx")
...
varicela <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/evento 831 datos basicos SEM 07.xlsx")
...
epi3_1 <- epi3 %>% 
  ...
  filter(as.numeric(semana)<=7)      # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general se encuentra por debajo del umbral estacional (Figura 2). La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
Por grupo de edad en la hospitalizaciones por IRA, la mayor concentración de hospitalizaciones a causa de infección respiratoria se da en el grupo de edad menores e iguales a un año.
...
varicela_ajus <- expand.grid(semana = 1:7, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
