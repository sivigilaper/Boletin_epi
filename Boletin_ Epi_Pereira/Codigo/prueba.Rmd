...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general  muestra disminución continua, presentandose sobre el umbral estacional. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
valor <- predict.glm(model, newdata = data.frame(t= c(43)),type = "response") #cambiar a cada semana
