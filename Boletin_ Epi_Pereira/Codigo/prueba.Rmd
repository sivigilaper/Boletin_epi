...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 4 - 2023} 
...
epi3_1 <- epi3 %>% 
  ...
  filter(as.numeric(semana)<=4)      # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
...
plot(ira$semana, ira$icinfuci, type='l', ylim=c(0,25), xlim=c(1,52), col="green",lwd=3, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
En cuanto a la edad, en la consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se observa una concentración del mas de 10% en los menores de 4 años, con una mayor concentración de la consulta por IRA entre entre los menores a 1 año. El canal endemico general para IRA sobre un umbral estacional
...
```{r, echo=FALSE, warning=FALSE, fig.height=3 , fig.width=7, fig.cap="Positividad muestras tracto respiratorio, Pereira 2022"}
filmarray %>% 
  group_by(semana) %>% 
...
```{r,echo=FALSE, warning=FALSE}
covid <- epi1 %>% filter(cod_eve == "346") %>% group_by(tip_cas_) %>% count()
```

Durante este año 2023, se han notificado a semana epidemiologica `r ncol(casos_sem)` al sistema de vigilancia SIVIGILA un total de `r sum(casos_sem["346",1:ncol(casos_sem)])` casos leves o sin necesidad de hospitalización.

```{r, echo=FALSE, warning=FALSE, fig.height=3 , fig.width=7, fig.cap="Notificacion semanal COVID-19, Pereira 2023"}
ggplot(epi1 %>% filter(cod_eve=="346"))+
...
varicela_ajus <- expand.grid(semana = 1:4, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
