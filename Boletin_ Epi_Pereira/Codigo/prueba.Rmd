...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=epiweek(today()-7)) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), con aumento para esta semana epidemiologica notificada. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
según canal endémico (figura 5) se identifica dengue con cambios importantes de aumento en las ultimas dos semanas epidemiologicas, sobre pasando el limite superior para semana 44 y 45. 
