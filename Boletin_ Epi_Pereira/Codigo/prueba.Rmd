...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 38 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=38) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
#   summarise(sum(tot_ira)) %>%
#   ungroup() %>%
#   pivot_wider(names_from = semana, values_from = `sum(tot_ira)`)
```
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana se encuentra sobre umbral estacional. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), con descenso para esta semana epidemiologica notificada. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
según canal endémico (figura 5) se identifica dengue estable sin cambios importantes sobre nivel de alerta y zona de control del evento.
