...
subtitle: "Alertas Tempranas Semana Epidemiologica 4 de 2022"
header-includes:
...
En relación a la notificación segun servicio y comparando la semana notificada del actual año con la misma semana del año anterior, se identifica una disminución en la hospitalización IRA en UCI, pero con aumentos relativos de $12.4 \%$  y $67\%$ en sala general y urgencias, respetivamente.
...
En la semana epidemiológica `r ncol(casos_sem)` de 2022 se  notificaron  0   muertes perinatales;  casos acumulados hasta el momento.

```{r, echo=FALSE}
#`r casos_sem["560",ncol(casos_sem)]`
#`r sum(casos_sem["560",1:ncol(casos_sem)])` 
```
...
----------|:-----------------:|:-----------:|:-----------:|
Int. suicida | `r casos_sem["356",ncol(casos_sem2)] ` | `r round(as.numeric(histo[11,ncol(casos_sem)+1]),digits=0) `|
`r round(dpois(casos_sem["356",ncol(casos_sem)], as.numeric(histo[11,ncol(casos_sem)+1])), digits=2) ` |
ETA | 0 | 0 | 0|
...
## Tuberculosis 
Se  notificaron al sistema de vigilancia para esta semana 0 casos de tuberculosis sensible, con un acumulado en el sivigila de  0  casos  para TB sensible.

```{r, echo=FALSE}
#`r casos_sem2["813",ncol(casos_sem2)] `
#`r sum(casos_sem2["813",1:ncol(casos_sem2)]) `
...
Para esta semana epidemiológica `r ncol(casos_sem) ` de 2022 se notificaron al sistema de vigilancia  0 casos de defecto congénito, y  `r casos_sem2["110", ncol(casos_sem2)]`  casos de bajo peso al nacer, siendo este último catalogado como de alta incidencia.  El total de casos ingresados hasta el momento son: 0 casos para defectos congénitos y `r sum(casos_sem2["110",1:ncol(casos_sem2)])` casos para bajo peso al nacer.
