...
En cuanto a la edad, en las consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se muestra en la tabla. El canal endemico general para IRA esta en el umbral estacional en zona de brote.
...
Durante el periodo comprendido entre las semanas epidemiológicas 11 y 13, la dinámica de circulación viral muestra un predominio marcado de los virus pertenecientes a la familia Picornaviridae. El Rhinovirus se posiciona como el principal agente etiológico con una proporción de incidencia global del 43.9% ($IC95\%: 38.17 - 49.76$), aunque se observa una tendencia a la disminución progresiva al comparar el primer intervalo (SE 11-10) donde registró un 60.0%, frente al segundo intervalo (SE 12-13) donde descendió al 52.2%. De manera similar, el Enterovirus mantuvo una presencia importante pero con una reducción moderada, pasando de una incidencia semanal del 31.4% a un 26.1%. En contraste con este descenso de los virus predominantes, se advierte un incremento epidemiológicamente relevante de Influenza A, que experimentó un repunte significativo al elevarse de un 5.7% en la medición inicial a un 17.4% en el cierre del periodo, consolidándose como la tercera causa de aislamiento con un total de 32 casos.
...
Por otro lado, agentes como el Virus Sincitial Respiratorio (VSR) y la Influenza AH1N1 mostraron un comportamiento de cese en su detección hacia las semanas 12-13, cayendo ambos a una incidencia del 0.0% tras haber iniciado el periodo con un 5.7% y 2.9% respectivamente. Esta ausencia de detecciones finales contrasta con el comportamiento de otros virus emergentes o estables en la vigilancia, como el Adenovirus, que incrementó su actividad del 2.9% al 8.7%, y el Parainfluenza, que tras un silencio inicial alcanzó una incidencia del 8.7% al cierre del reporte. Finalmente, se identifica una circulación marginal y esporádica de Coronavirus (4.0%), Metaneumovirus (1.1%) y SARS-CoV-2 (0.4%), este último con un único aislamiento registrado, lo que sugiere una baja presión de este patógeno en el perfil epidemiológico actual frente al resto de virus respiratorios estacionales.
...
valor <- predict.glm(model, newdata = data.frame(t= c(14)),type = "response") #cambiar a cada semana
...
lines(x = 14, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
