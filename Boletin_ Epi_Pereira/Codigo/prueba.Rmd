...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general se encuentra por debajo del umbral estacional (Figura 2), se registro un incremento en las dos semanas anteriores. La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
varicela_ajus <- expand.grid(semana = 1:11, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
