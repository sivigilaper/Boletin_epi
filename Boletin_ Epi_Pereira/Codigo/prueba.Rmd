...
subtitle: "Alertas Tempranas Semana Epidemiologica 9 de 2022"
lang: es
header-includes:
- \usepackage{graphicx}
- \usepackage{fancyhdr}
...
A semana epidemiológica `r ncol(casos_sem)` de 2022 se notificaron un total de `r format(sum(ira2018[1:ncol(casos_sem),"casos"]), scientific=F)` consultas por IRA procedentes de los servicios de hospitalización en sala general, UCI, consulta externa y urgencias. El canal endémico muestra el evento para la semana `r ncol(casos_sem)` se ubica por encima del limite superior.
...
En consulta externa y urgencias la notificación de infección respiratoria aguda ha estado en forma variable no completando dos semanas seguidas por encima de limite superior. Se registra un aumento relativo en relación al año inmediatamente anterior.
...
| Sarampion           |  `r casos_sem["730",ncol(casos_sem)]`   | `r round(as.numeric(histo[22,ncol(casos_sem)+1]), digits=0)` |  `r round(dpois(0, as.numeric(histo[22,ncol(casos_sem)+1])), digits=2)`                 |
| Tosferina           |          0   |     0 |      0       |
