...
- \fancyhead[R]{Secretaria de Salud Pública y Seguridad Social \\ Vigilancia en Salud Pública \\ Semana 25 - 2023} 
...
epi3 <- epi3 %>% 
  filter(as.numeric(semana)<=25) %>%       # SIEMPRE CAMBIAR A LA SEMANA NOTIFICADA DEL BOLETIN
  mutate(semana = as.numeric(semana))
...
Para la semana `r ncol(casos_sem)` la hospitalización por IRA en sala general para esta semana presenta comportamiento inusual, lo cual esta en verificación (Figura 2). La proporción acumulada de hospitalización por IRA es de  `r round(sum(evento_995$tot_irag)/sum(evento_995$tot_cau_h)*100,digits=2)`%.
...
aislamiento %>% 
  ...
  ggplot(aes(x = semana, y = Aislamientos,
           ...
           geom_col()+
           scale_x_continuous(limits = c(0,53), breaks = seq(1,52,1))+
           labs(x = "Semana epidemiologica", y = "Aislamientos (n)")+ theme_minimal()+
...
varicela_ajus <- expand.grid(semana = 1:25, sexo_=c("F","M"), gr_edad=c("0-9","10 a 19","20 a 40","Mayores a 40"))
