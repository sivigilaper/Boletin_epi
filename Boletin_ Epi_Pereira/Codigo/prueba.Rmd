...
evento_995 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/2024_995.xlsx")
evento_998 <- read_excel("/cloud/project/Boletin_ Epi_Pereira/Datos/2024_998.xlsx")
...
legend(25,4000, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "casos 2024"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```
...
| **Servicio**               |                 **a Sem `r ncol(casos_sem)`/2023**                  |                 **a Sem `r ncol(casos_sem)`/2024**                 |                                                                      **Variacion (%)**                                                                      |
|-----------------|:---------------:|:---------------:|:-------------------:|
...
```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en sala general , Pereira 2024", fig.height=3.3, fig.width=7}
...
legend(5,160, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "casos 2024"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

De igual manera en el canal endémico para la notificación por IRA hospitalizados en Unidad de Cuidados Intensivos (**UCI**) (figura 3), sobre el umbral estacional a semana epidemiologica notificada. La proporción acumulada de hospitalizaciones por IRA en UCI alcanza un `r round(sum(evento_995$tot_irauci)/sum(evento_995$tot_h_uci)*100,digits=2)`%.
...
```{r, echo=FALSE, fig.cap="Canal endémico Hospitalizaciones por IRA en UCI , Pereira 2024", fig.height=3.3, fig.width=7}
...
legend(8,15, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2024"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.5)
```
...
```{r, echo=FALSE, fig.cap="Canal endémico consulta externa y urgencias por IRA, Pereira 2024", fig.height=3.3, fig.width=7}
...
legend(25,3800, c("IC Inf 95%","IC Sup 95%", "Umbral estacional", "2024"), col=c("green","red","yellow","black"), lty = c(1,1), lwd = c(2,2), bty="n", cex=0.6)
```

\newpage

En cuanto a la edad, en las consultas por IRA en servicio de consulta externa y urgencias, la proporción de consultas por IRA en cada grupo de edad se observa una concentración del mas de 10% en los de un año. El canal endemico general para IRA se encuentra por debajo de umbral estacional.
...
aislamiento <- aislamientos %>% 
  ...
  filter(year(FECHA) == 2024) %>%  # filtro de año 2024
...
positivo <- filmarray2 %>% 
  filter(year(FECHA) == 2024) %>%    # filtro de año de muestra
  group_by(resultado) %>% 
  summarise(n=n()) %>% 
...
```{r, echo=FALSE, warning=FALSE, fig.height=2.5 , fig.width=7, fig.cap="Positividad muestras tracto respiratorio, Pereira 2024"}
filmarray2 %>% 
  filter(year(fec_muestra)==2024) %>% # filtra solo para 2024
  group_by(semana) %>% 
...
```
\newpage
A periodo epidemiologico 1 de 2024 `r nrow(filmarray2 %>% filter(year(FECHA)==2024)) ` muestras en el municipio, con aislameintos tanto virales como bacterianos con una positividad acumulada de `r positivo[2,2] `  y una  positividad semanal como lo muestra la **figura 5**. La incidencia de microorganismos en aislamientos virales procesado por FilmArray para lo corrido del año son:
...
De esta manera se estima que la incidencia acumulada (entre muestras procesadas) para microorganismos asilados a través de panel molecular viral rapido en orden de aparición son  Influenza AH1N1, Rinovirus, caracterizado este pico estacional de Infeccion respiratoria con una probable mayor circulación de Influenza AH1N1.
...
```{r, echo=FALSE, warning=FALSE, fig.height=3.2 , fig.width=7, fig.cap="Circulación virus respiratorios, Pereira 2024"}
aislamiento %>% 
  mutate(viral = case_when(value == "Enterovirus" ~ "Enterovirus",
                           value == "enterovirus" ~ "Enterovirus",
                           value == "enterovirus humano" ~ "Enterovirus",
  ...
  filter(year(FECHA) == 2024) %>% # filtra AÑO
...
```{r, echo=FALSE, fig.height=3, fig.width=7, fig.cap="Canal endémico Dengue, Pereira 2024"}
plot(canalend$semana, canalend$exito, type='l', ylim=c(0,70), xlim=c(1,52), col="green",lwd=2, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
lines(x = 10, y = mean(casos_sem2['210',(ncol(casos_sem2)-3):ncol(casos_sem2)-1]), col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
legend(30,70, c("IC inf 95%","Media", "IC sup 95%", "2024","Ajuste por rezago"), col=c("green","yellow","red","black","blue"), lty = c(1,1,1,1,1), lwd = c(1,1), bty="n", cex=0.6)
```
...
legend(25,1400, c("IC inf 95%","Media", "IC sup 95%", "2024"), col=c("green","yellow","red","black"), lty = c(1,1), lwd = c(1,1), bty="n", cex=0.5)
```
...
En la semana epidemiológica `r ncol(casos_sem)` de 2024 se notificaron `r casos_sem["560",ncol(casos_sem)]` muertes perinatales; `r sum(casos_sem["560",1:ncol(casos_sem)])` casos acumulados hasta el momento.
...
En la semana epidemiológica `r ncol(casos_sem)` de 2024 se notificaron al Sivigila `r casos_sem2["300",ncol(casos_sem2)]` casos de agresiones por animales potencialmente transmisores de rabia , con un acumulado hasta el momento de `r sum(casos_sem2["300",1:ncol(casos_sem2)])` casos.
...
Para esta semana epidemiológica `r ncol(casos_sem)` de 2024 se notificaron al sistema de vigilancia `r casos_sem["215", ncol(casos_sem)]` casos de defecto congénito. El total de casos ingresados hasta el momento son: `r sum(casos_sem["215",1:ncol(casos_sem)])` casos para defectos congénitos.
...
Durante este año 2024, se han notificado a semana epidemiologica `r ncol(casos_sem)` al sistema de vigilancia SIVIGILA un total de `r covid[which(covid$tip_cas_=="3"),"n"]` casos leves o sin necesidad de hospitalización.

```{r, echo=FALSE, warning=FALSE, fig.height=3 , fig.width=7, fig.cap="Notificacion semanal COVID-19, Pereira 2024"}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Comportamiento de varicela para Hombres por grupos de edad, Pereira 2024"}
####################################################################################################
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Comportamiento de varicela para Mujeres por grupos de edad, Pereira 2024"}
