...
evento_998<-evento_998 %>%
  ...
  filter(ajuste!= "D") %>%
  mutate(semana= as.numeric(semana)) %>%
  mutate(cas_conc= as.numeric(cas_conc)) %>% filter(semana<16) #### QUITAR SEMANA 16
diarreas<- evento_998 %>% 
  group_by(semana) %>% 
  summarise(
    casos = sum(cas_conc))
...
plot(canalend$semana, canalend$exito, type='l', ylim=c(0,130), xlim=c(1,52), col="green",lwd=2, xlab="Semana epidemiológica", ylab="Casos", cex=0.6, main = "")
...
lines(x = 17, y = valor, col = "blue", type = "p", lwd=4, pch = 18) # ajuste por rezago en la notificaicon por media movil de los ultimas 5 semanas
